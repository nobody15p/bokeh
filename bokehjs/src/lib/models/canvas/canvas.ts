...
export class CanvasView extends UIElementView {
  ...
  override _after_resize(): void {
    ...
    const changed = this.pixel_ratio_changed || width != this._last_size.width || height != this._last_size.height

    if (changed) {
      this._last_size = new BBox({x: 0, y: 0, width, height})
